...
class DolphinConfig:
    ...
    def set_ff(self, enable=True):
        ...
        ff_line = 20 # 18
        if "Fast Forward" not in data[ff_line]:
            raise FileNotFoundError(f"Error: cannot locate Fast Forward Gecko code in "
                 ...
                 "file, and that it is on line {ff_line+1}!")

        if data[ff_line][0] == "-" and enable: 
            data[ff_line] = "$Optional: Fast Forward\n"
            with open(str(self.slippi_gecko_ini_path), 'w') as f:
                f.writelines(data)

        elif data[ff_line][0] == "$" and not enable:
            data[ff_line] = "-Optional: Fast Forward\n"
            with open(str(self.slippi_gecko_ini_path), 'w') as f:
                f.writelines(data)
    ...
    def set_center_p2_hud(self, enable=True):
        ...
        ff_line = 18 # 16
        if "Center Align 2P HUD" not in data[ff_line]:
            raise FileNotFoundError(f"Error: cannot locate Fast Forward Gecko code in "
                 "{self.slippi_gecko_ini_path}, please ensure it is located in this "
                 "file, and that it is on line 19!")

        if data[ff_line][0] == "-" and enable: 
            data[ff_line] = "$" + data[ff_line][1:]
            with open(str(self.slippi_gecko_ini_path), 'w') as f:
                f.writelines(data)
    ...
    def install_slippi(self, install_path):
        if self.platform == "linux":
            ...
            target_url = "https://github.com/project-slippi/Ishiiruka/releases/download/v3.4.0/FM-Slippi-3.4.0-Win.zip"
            # raise NotImplementedError("Windows currently not supported at this time.")

        elif self.platform == "darwin":
            target_url = "https://github.com/project-slippi/Ishiiruka/releases/download/v3.4.0/FM-Slippi-3.4.0-Mac.dmg"
        ...
        print("target:",install_path / slippi_game_path.name)
        temp = install_path / slippi_game_path.name
        slippi_game_path = slippi_game_path.rename(install_path / slippi_game_path.name)
        slippi_game_path = temp
        print("slippi:",slippi_game_path)
    ...
    def apply_gecko_codes(self, install_path):
        ...
        empty_line = 16 # 14
        if data[empty_line-1] == "$Recommended: Apply Delay to all In-Game Scenes\n":
            data[empty_line-1] = "-" + data[empty_line-1][1:]  # turn this off
        
        # check that line 15 is empty before we start adding stuff
        
        if data[empty_line] != "\n":
            raise FileNotFoundError(f"Something has gone wrong... check that {self.slippi_gecko_ini_path} exists.")

        data.insert(empty_line, "-Optional: Fast Forward\n")
        data.insert(empty_line, "-Optional: Flash Red on Failed L-Cancel\n")
        data.insert(empty_line, "$Optional: Center Align 2P HUD\n")
        data.insert(empty_line, "$Optional: Disable Screen Shake\n")
        data.insert(empty_line, "-Optional: Widescreen 16:9\n")
